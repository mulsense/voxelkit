    ...
    class Model {
        ...
        convertVRM() {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                for (let i = 0; i < this.layers.length; i++) {
                    ...
                    for (let j = offset; j < vertexs.length / 3; j++) {
                        ...
                        if (nid0 >= 0) {
                            ...
                            for (let k = 1; k < this.bones.length; k++) {
                                if (k !== nid0 && mask[k]) {
                                    if (this.bones[nid0].name.indexOf('left') === 0 && this.bones[k].name.indexOf('right') === 0)
                                        continue;
                                    if (this.bones[nid0].name.indexOf('right') === 0 && this.bones[k].name.indexOf('left') === 0)
                                        continue;
                                    const l = this.bones[k].distance(vertexs.slice(j * 3, j * 3 + 3));
                                    if (l < min1) {
                                        min1 = l;
                ...
                {
                    let offset = 0;
                    bufferViews.push({ buffer: 0, byteOffset: offset, byteLength: indices.length * 4, target: 34963 });
                    offset += indices.length * 4;
                    bufferViews.push({ buffer: 0, byteOffset: offset, byteLength: vertexs.length * 4, target: 34962 });
                    offset += vertexs.length * 4;
                    bufferViews.push({ buffer: 0, byteOffset: offset, byteLength: normals.length * 4, target: 34962 });
                    offset += normals.length * 4;
                    bufferViews.push({ buffer: 0, byteOffset: offset, byteLength: coords.length * 4, target: 34962 });
                    offset += coords.length * 4;
