...
function convertGLB(models) {
    ...
    for (const model of models) {
        ...
        for (const [key, corners] of groups) {
            ...
            accessors.push({ bufferView: addView(bytesOf(new Float32Array(positions)), 34962), componentType: 5126, count: vertexCount, type: 'VEC3', min, max }, { bufferView: addView(bytesOf(new Float32Array(normals)), 34962), componentType: 5126, count: vertexCount, type: 'VEC3' }, { bufferView: addView(bytesOf(indices), 34963), componentType: 5125, count: indices.length, type: 'SCALAR' });
            const base = accessors.length - 3;
            primitives.push({ attributes: { POSITION: base, NORMAL: base + 1 }, indices: base + 2, material, mode: 4 });
        }
        const name = objectName$1(model.name, nodes.length);
...
function objectName$1(name, index) {
 ...
 */
function convertOBJ(models, mtlName = 'model.mtl') {
...
function objectName(name, index) {
...
const voxelkit = {
     ...
     */
    convertOBJ(composit, { mtlName = 'model.mtl' } = {}) {
